...
/// Fq12 element a+bw is represented as a b on the stack where a and b are fq6 elements.. (b is close to the top of the stack) w^2=v
/// Accessing an `fq12` is implemented by accessing its first `fq` element on the stack.
/// For example, to add the first two `fq12` elements, use `add(0, 12)` instead of `add(0, 1)`.
pub struct Fq12;
