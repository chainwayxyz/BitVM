...
/* 
/// BLAKE3 for msg_len number of bytes, message is expected in nibbles (significant part being first), first element being at the start
pub fn blake3_script(msg_len: u32) -> Script {
    assert!(msg_len % 4 == 0, "Byte count needs to be a multiple of four");
    let mut stack = StackTracker::new();
    blake3(&mut stack, msg_len as u32, 8);
    stack.get_script()
}

/// In bitvm, 32 bit numbers are represented as their bytes reversed
pub fn blake3_bitvm_version(mut v: Vec<u8>) -> [u8; 32] {
...
// This is just normal BLAKE3 now 
